$(document).ready(function () {
    document.title = 'Warehouse Notification';

    $("#tblWarehouseNotification").DataTable({
        paging: true,
        select: true,
        "order": [[0, "desc"]],
        dom: 'Bfrtip',


        buttons: [
            'pageLength',
        ],


        "processing": true,
        "serverSide": true,
        "filter": true, //Search Box
        "orderMulti": false,
        "stateSave": true,

        "ajax": {
            "url": "/WarehouseNotification/GetDataTabelData",
            "type": "POST",
            "datatype": "json"
        },


        "columns": [
            {
                data: "Id", "name": "Id", render: function (data, type, row) {
                    return "<a href='#' class='fa fa-eye' onclick=Details('" + row.Id + "');>" + row.Id + "</a>";
                }
            },
            { "data": "FromWarehouseDisplay", "name": "FromWarehouseDisplay" },
            { "data": "ToWarehouseDisplay", "name": "ToWarehouseDisplay" },
            { "data": "ItemId", "name": "ItemId" },
            { "data": "SendQuantity", "name": "SendQuantity" },
            { "data": "ReceiveQuantity", "name": "ReceiveQuantity" },
            { "data": "Message", "name": "Message" },

            {
                "data": "CreatedDate",
                "name": "CreatedDate",
                "autoWidth": true,
                "render": function (data) {
                    var date = new Date(data);
                    var month = date.getMonth() + 1;
                    return (month.length > 1 ? month : month) + "/" + date.getDate() + "/" + date.getFullYear();
                }
            },
            {
                data: null, render: function (data, type, row) {
                    return "<a href='#' class='btn btn-danger btn-xs' onclick=MarkedAsRead('" + row.Id + "'); >Marked as Read</a>";
                }
            }
        ],

        'columnDefs': [{
            'targets': [8],
            'orderable': false,
        }],

        "lengthMenu": [[20, 10, 15, 25, 50, 100, 200], [20, 10, 15, 25, 50, 100, 200]]
    });
});

