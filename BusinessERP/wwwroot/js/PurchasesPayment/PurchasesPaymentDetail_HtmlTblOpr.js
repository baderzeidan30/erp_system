var UpdateUnitPrice = function () {
  ...
  if (_IsVat == "Yes") {
    ...
    $("#ItemVAT").val(parseFloat(_ItemVAT));
  } else {
    $("#ItemVAT").val(0);
  }
...
var GeneratePriceDropdown = function (splitArray) {
  ...
  var ItemPriceData = {
    1: "Normal:" + splitArray[2],
    2: "Cost:" + splitArray[4],
    3: "Trade:" + splitArray[6],
    4: "Premium:" + splitArray[8],
  };
...
var UpdateItemAllElement = function () {
  ...
  var _ItemDiscountAmount =
    (parseFloat(_ItemDiscount) / 100) * parseFloat(_UnitPrice);

  _UnitPrice = parseFloat(_UnitPrice) - _ItemDiscountAmount;
  _UnitPrice =
    parseFloat(_UnitPrice) +
    (parseFloat(_ItemVAT) / 100) * parseFloat(_UnitPrice);
...
var AddHTMLTableRow = function (result) {
  var tBody = $("#tblPurchasesPaymentDetail > TBODY")[0];
  ...
  if (_IsVat == "Yes") {
    var cell = $(row.insertCell(-1));
    var txtItemVAT = $("<input />");
    txtItemVAT.attr("type", "number");
    txtItemVAT.attr("style", "width:50px;");
    txtItemVAT.attr("id", "ItemVAT" + result.Id);
    txtItemVAT.attr("min", "0");
    txtItemVAT.attr("onchange", "UpdateItemDynamicControl(this);");
    txtItemVAT.val($("#ItemVAT").val());
    cell.append(txtItemVAT);
  }
  ...
  btnUpdate.attr("onclick", "UpdatePurchasesPaymentDetail(this);");
...
function LoadTableRowFromDB(item, index) {
  ...
  if (_IsVat == "Yes") {
    ...
    txtItemVAT.attr("type", "number");
    txtItemVAT.attr("style", "width:50px;");
    txtItemVAT.attr("id", "ItemVAT" + item.Id);
    txtItemVAT.attr("min", "0");
    txtItemVAT.attr("onchange", "UpdateItemDynamicControl(this);");
    txtItemVAT.val(item.ItemVAT);
    cell.append(txtItemVAT);
  }
...
function Remove(button) {
  Swal.fire({
    ...
    if (result.value) {
      ...
      var table = $("#tblPurchasesPaymentDetail")[0];
      table.deleteRow(row[0].rowIndex);
      var _PurchasesPaymentDetailId = $("TD", row).eq(0).html();

      var item = {};
      item.Id = _PurchasesPaymentDetailId;
      item.PaymentId = $("#Id").val();
      DeletePurchasesPaymentDetail(item);
    }
  });
}
...
var UpdateItemDynamicControl = function (button) {
  ...
  var _NormalVAT;
  if (_IsVat == "Yes") {
    _NormalVAT = $("#ItemVAT" + _ItemId).val();
  } else {
    _NormalVAT = 0;
  }
  ...
  var _DiscountedTotalPrice =
    parseFloat(_TotalPrice) - parseFloat(_DiscountTotalAmount);

  var _TotalPriceWithVAT =
    parseFloat(_DiscountedTotalPrice) +
    (parseFloat(_NormalVAT) / 100) * parseFloat(_DiscountedTotalPrice);
