...
var UpdateUnitPrice = async function () {
...
var onchangeUnitPrice = async function () {
    await onchangeQuantity();
}

var onchangeQuantity = async function () {
    var _Quantity = parseFloat($("#Quantity").val());
    var _UnitPrice = $("#UnitPrice option:selected").text();
    var splitUnitPrice = _UnitPrice.split(":");
    _UnitPrice = (parseFloat(splitUnitPrice[1])).toFixed(2);
    var _ItemDiscount = parseFloat($("#ItemDiscount").val());
    var _ItemVAT = parseFloat($("#ItemVAT").val());

    var _ItemDiscountAmount = 0;
    var _CompanyInfo = await GetCompanyDataFromLS();
    if (_CompanyInfo.IsItemDiscountPercentage == true) {
        _ItemDiscountAmount = (_ItemDiscount / 100) * _UnitPrice;
    }
    else {
        _ItemDiscountAmount = _ItemDiscount;
    }

    _UnitPrice = _UnitPrice - _ItemDiscountAmount;
    _UnitPrice = (_UnitPrice + (_ItemVAT / 100) * _UnitPrice).toFixed(2);

    var _TotalAmount = parseFloat(_UnitPrice * _Quantity);
    $("#TotalAmount").val(_TotalAmount.toFixed(2));
}

var onchangeItemDiscount = async function () {
    await onchangeQuantity();
}
