    ...
    public class AccountController : Controller
    {
        ...
        public async Task<JsonResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            try
            {
                ...
                {
                    _JsonResultViewModel.AlertMessage = "Requires Two Factor Authentication.";
                }
                else
                {
        ...
        private async Task InserLoginHistory(bool IsLoginAction, bool _IsSuccess, LoginViewModel _LoginViewModel)
        {
            ...
            else
            {
                ...
                var _LoginHistory = _context.LoginHistory.Where(x => x.UserName == _UserName && x.Action == "Login").OrderByDescending(x => x.CreatedDate).Take(1).FirstOrDefault();
