    ...
    public class DashboardController : Controller
    {
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                vm.LowItems = _Items.Where(x => x.Quantity < 10).Count();
                vm.OutofStock = _Items.Where(x => x.Quantity == 0).Count();
                vm.DueInvoice = _Payment.Where(x => x.DueAmount > 0).Count();
        ...
        private List<RecentInvoiceViewModel> GetRecentInvoicesList()
        {
            try
            {
                //CultureInfo enCultureInfo = new CultureInfo("en");
                ...
                var result = (from obj in _context.Payment
                              ...
                              where (obj.Cancelled == false && (new[] { InvoiceType.RegularInvoice, InvoiceType.ManualInvoice }).Contains(obj.Category))
