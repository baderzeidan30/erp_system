...
namespace BusinessERP.Controllers
{
    ...
    public class SalesReportController : Controller
    {
        ...
        [HttpGet]
        public JsonResult GetSummaryReportData(string DateType)
        {
            try
            {
                List<SummaryReportViewModel> list = new();
                list = _iSalesService.GetSummaryReportData(DateType);
        ...
        public IActionResult PaymentSummaryReport()
        {
            try
            {
                var result = _iCommon.GetTableData<Branch>(x => x.Cancelled == false).ToList();
                ViewBag.ddlBranch = new SelectList(result.ToList(), "Id", "Name");
                return View();
            }
            catch (Exception) { throw; }
        }
        ...
        [Authorize(Roles = MainMenu.AttendanceReport.RoleName)]
        [HttpGet]
        ...
        [HttpPost]
        public IActionResult AttendanceReportDataTable(bool IsFilterData, Int64 EmployeeId)
        {
            try
            {
                var draw = HttpContext.Request.Form["draw"].FirstOrDefault();
                ...
                IQueryable<AttendanceCRUDViewModel> _GetAttendanceReportData;
                if (IsFilterData)
                {
                    _GetAttendanceReportData = _iCommon.GetAttendanceReportData().Where(obj => obj.EmployeeId == EmployeeId);
                }
                else
                {
                    _GetAttendanceReportData = _iCommon.GetAttendanceReportData();
                }

                //Sorting
                if (!(string.IsNullOrEmpty(sortColumn) && string.IsNullOrEmpty(sortColumnAscDesc)))
                {
                    _GetAttendanceReportData = _GetAttendanceReportData.OrderBy(sortColumn + " " + sortColumnAscDesc);
                }
                ...
                {
                    searchValue = searchValue.ToLower();
                    _GetAttendanceReportData = _GetAttendanceReportData.Where(obj => obj.Id.ToString().Contains(searchValue)
                ...
                resultTotal = _GetAttendanceReportData.Count();
                var result = _GetAttendanceReportData.Skip(skip).Take(pageSize).ToList();
                return Json(new { draw = draw, recordsFiltered = resultTotal, recordsTotal = resultTotal, data = result });
        ...
        public JsonResult GetEmployeeddlList()
        {
            try
            {
                var result = _iCommon.LoadddlEmployee().ToList();
